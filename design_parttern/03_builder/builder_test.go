...
func TestResourcePoolConfigBuilder_Build(t *testing.T) {
	...
	configBuilder := ResourcePoolConfigBuilder{}

	configBuilder.SetName("test")
	configBuilder.SetMaxTotal(100)
	configBuilder.SetMaxIdle(90)
	configBuilder.SetMinIdle(80)

	config, err := configBuilder.Build()

	assert.Nil(t, err)
	assert.Equal(t, config.name, "test")
	assert.Equal(t, config.maxTotal, 100)
	assert.Equal(t, config.maxIdle, 90)
	assert.Equal(t, config.minIdle, 80)
}
